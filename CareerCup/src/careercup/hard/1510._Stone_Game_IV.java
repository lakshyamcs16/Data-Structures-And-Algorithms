...
class Solution {
 ...
        return minimax4(n, dp);
        
    }
    //5
    private boolean minimax4(int n, boolean[] dp) {
        //base case
        if(n == 0) {
            return false;
        }
 ...
        if(dp[n]) return dp[n];
        boolean ans = false;
        //traverse through every possibility for a player but stop if we find a winning move
        for(int i=1; !ans && i*i<=n; i++) {
            //try to pick the one where opponent fails (returns false)
            ans = !minimax4(n - i*i, dp);
        }
        
        return dp[n]=ans;
    }
